...
export function AuthForm({ mode }: { mode: 'sign-in' | 'sign-up' | 'forgot-password' | 'reset-password' }) {
  ...
  /** The email of an unverified account, set when sign-in is refused so the
      resend button below knows where to send the new verification link. */
  ...
  async function resendVerification() {
    ...
    setResending(false)
    if (result?.error) { setError('تعذر إعادة إرسال رسالة التأكيد. حاول مرة أخرى بعد قليل.'); return }
    setError('')
    setMessage('أعدنا إرسال رسالة التأكيد. تحقق من صندوق الوارد (ومجلد الرسائل غير المرغوبة).')
  }

  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setMessage(''); setUnverifiedEmail(''); setLoading(true)
    ...
    setLoading(false)
    if (result?.error) {
      console.error('[v0] Authentication request failed:', result.error.message)
      if (result.error.code === 'EMAIL_NOT_VERIFIED' || /not verified/i.test(result.error.message ?? '')) {
        setUnverifiedEmail(email)
        setError('لم يتم تأكيد بريدك الإلكتروني بعد. اضغط على الرابط الذي أرسلناه إليك ثم سجّل الدخول.')
      } else {
        setError('تعذر إتمام العملية. تحقق من البيانات وحاول مجددًا.')
      }
      return
    }
    ...
    /**
     * With email verification required, a successful sign-up creates the
     * account but NOT a session — redirecting to /projects would bounce
     * straight back here. The account exists; the only step left is the
     * email link, so say that instead of navigating.
     */
    if (mode === 'sign-up') { setMessage('أنشأنا حسابك. أرسلنا رسالة تأكيد إلى بريدك — اضغط على الرابط فيها ثم سجّل الدخول.'); return }
  ...
  return <form onSubmit={submit} className="mx-auto flex w-full max-w-md flex-col gap-4 rounded-2xl border border-border bg-card p-6 shadow-xl">
    ...
    {unverifiedEmail && <button type="button" onClick={() => void resendVerification()} disabled={resending} className="rounded-lg border border-input bg-background px-4 py-2 text-sm font-medium disabled:opacity-50">{resending ? 'جارٍ الإرسال...' : 'إعادة إرسال رسالة التأكيد'}</button>}
